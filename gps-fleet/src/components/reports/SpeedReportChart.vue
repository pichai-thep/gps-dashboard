<template>
  <div class="speed-chart">
    ...
    <svg v-else :viewBox="`0 0 ${width} 360`" :style="{ minWidth: `${width}px` }" role="img" :aria-label="t('speedChart')">
...
const points = computed(() => props.rows.map((row, index) => {
  ...
  return { id: String(value(row, 'gpsdata_id') ?? index), speed: Number(value(row, 'speed') ?? 0), time: date.slice(11, 16) || String(index + 1) }

...
BEGIN
    ...
    SELECT
        ...
        CASE
            WHEN t.ur_rate_type = 'A' THEN
                CASE
                    WHEN (t.ur_rate_sunday = 0
                          AND DAYOFWEEK(s.data_date) = 1)
                         OR (t.ur_rate_saturday = 0
                             AND DAYOFWEEK(s.data_date) = 7)
                    THEN NULL
                    ELSE ROUND(
    ...
    WHERE s.data_date BETWEEN p_date_from AND p_date_to
      ...
      /* ตรวจสิทธิ์โดยไม่ทำให้แถวข้อมูลถูกคูณ */
      AND EXISTS (
          SELECT 1

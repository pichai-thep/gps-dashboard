-- Adds the minimum distance (km) filter before summary and pagination.
...
CREATE PROCEDURE sp_report_daily_summary(
    ...
    IN p_date_to DATE,
    IN p_imeis TEXT,
    IN p_distance_from_km DECIMAL(12, 3),
    IN p_page INT,
    IN p_per_page INT,
...
BEGIN
    ...
    SELECT
        ...
        CASE
            WHEN t.ur_rate_type = 'A' THEN
                CASE
                    WHEN (t.ur_rate_sunday = 0 AND DAYOFWEEK(s.data_date) = 1)
    ...
    WHERE s.data_date BETWEEN p_date_from AND p_date_to
      ...
      AND (
             p_imeis IS NULL
          OR TRIM(p_imeis) = ''
          OR FIND_IN_SET(s.imei, p_imeis) > 0
      )
      AND (
             p_distance_from_km IS NULL
          OR s.distance_m >= p_distance_from_km * 1000
      );

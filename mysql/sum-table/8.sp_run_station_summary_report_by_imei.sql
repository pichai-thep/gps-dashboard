...
DROP PROCEDURE IF EXISTS sp_run_station_summary_report_by_imei $$
CREATE PROCEDURE sp_run_station_summary_report_by_imei(
...
proc: BEGIN
  ...
  WHILE v_date <= p_date_to DO
    CALL sp_sum_station_report_table_core(v_table_no, v_date, NULL, TRIM(p_imei));
...
-- Example: rebuild one IMEI's station summaries for an inclusive date range.

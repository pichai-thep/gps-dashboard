...
DROP PROCEDURE IF EXISTS sp_run_station_summary_report_by_customer $$
CREATE PROCEDURE sp_run_station_summary_report_by_customer(
...
proc: BEGIN
  ...
  DROP TEMPORARY TABLE IF EXISTS tmp_station_customer_report_tables;

  CREATE TEMPORARY TABLE tmp_station_customer_report_tables (
  ...
  INSERT INTO tmp_station_customer_report_tables (table_no)
  SELECT DISTINCT
    CASE
  ...
  SELECT COUNT(*)
    INTO v_table_count
  FROM tmp_station_customer_report_tables;
  ...
  WHILE v_date <= p_date_to DO
    ...
    WHILE v_row_no <= v_table_count DO
      ...
      CALL sp_sum_station_report_table_core(v_table_no, v_date, p_customer_id, NULL);
...
-- Example: rebuild station summaries for one customer and inclusive date range.
